...
const TOGGLE_KEYS = ["isAddressForeground", "isSearchForeground", "isSaveImage", "isPreferSaveImage"];

const TOGGLE_DEFAULTS = Object.freeze({
  isAddressForeground: true,
  isSearchForeground: true,
  isSaveImage: true,
  isPreferSaveImage: true,
});
...
const PATTERNS = {
  ...
  // h?tt は先頭の h が欠けたコピペずれ（ttp://）に対応し、( +)? は img src 属性の先頭スペースを許容する
  // hxx は hxxps:// などリンク難読化パターンに対応する
  SCHEME: /^(?:(?:( +)?h?tt|hxx)ps?|ftp|chrome|file):\/\//i,

  // メールアドレス判定
  EMAIL: /^[\w.+-]+@[\w.-]+\.[\w-]{2,}$/,

  // ttp://, ttps://, hxxp://, hxxps:// など難読化・コピペずれを http(s):// に正規化するためのパターン
  MALFORMED_HTTP: /^(?:t?t|h[tx]{2,})p(s?:\/\/)/i,

  // 先頭の不要文字除去
  LEADING_JUNK: /^:*[/\\\s]*/,

  // スキームなし ftp.xxx を http:// に統一した後 ftp:// に変換し直すためのパターン（#normalizeURL で使用）
